...
for spine in plt.gca().spines:						# python基础语法里，循环对象是字典时默认调用其key()函数
	if spine == "top" or spine == "right" :			# plt.gca()可以获得当前的子图。在pyplot模块中，许多函数都是对当前Figure或Axes对象进行处理。比如说，
		plt.gca().spines[spine].set_color("none")	# plt.plot()实际上会通过plt.gca()获取当前Axes对象ax，然后再调用ax.plot()绘图
...
# set annotating info
plt.annotate("maximum", xy=(np.pi/2, 1.0), xytext=((np.pi/2 + 0.15), 1.5), weight="bold", color="r", arrowprops=dict(arrowstyle="->", 
				connectionstyle="arc3", color="r"))

plt.annotate("spine", xy=(0.75, -3), xytext=(0.35, -2.25), weight="bold", color="b", arrowprops=dict(arrowstyle="->", 
				connectionstyle="arc3", color="b"))

plt.annotate("", xy=(0.0, -2.78), xytext=(0.4, -2.32), arrowprops=dict(arrowstyle="->", connectionstyle="arc3", color="b"))
